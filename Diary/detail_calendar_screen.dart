                                        ...
                                        Container(
                                          ...
                                          child: ElevatedButton(
                                            ...
                                            child: Text("Save", style: TextStyle(fontSize: 15.28, color: Colors.white, fontFamily: 'Poppins', fontWeight: FontWeight.w700),
                                              textAlign: TextAlign.center,),
                                            onPressed: () {
                                              if(changedDiary.isNotEmpty){
                                                document.reference.set({'contents': changedDiary}, SetOptions(merge: true)); // 바뀐 일기 데이터 저장
                                              }
                                              if(changedEmoji.isNotEmpty){
                                                document.reference.set({'emotion': changedEmoji}, SetOptions(merge: true)); // 바뀐 일기 데이터 저장
                                              }
                                              Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => TemperCalendar(email: widget.email))); // 캘린더 이동
                                            },
                                          ),
                                        )
                                      ]);
